...
def main():
    ...
    parser.add_argument("--camera-id", type=int, default=0,
                        help="OpenCV camera id for --input camera (default: 0)")
    parser.add_argument("--no-camera-preview", action="store_true",
                        help="Disable OpenCV hand-tracking preview for --input camera")
    ...
    if args.input == "noitom":
        ...
        input_device = Camera(
            ...
            show_preview=not args.no_camera_preview,
        )
        input_type = "camera"
    ...
    latest_qpos = None
    last_no_hand_report = 0.0
    data_lock = threading.Lock()
    stop_event = threading.Event()

    def process_fingers_data(fingers_data):
        nonlocal latest_raw_mediapipe_kp, latest_retarget_input_kp, latest_qpos
        nonlocal last_no_hand_report

        raw_kp = fingers_data[f"{args.hand}_fingers"]
        if np.allclose(raw_kp, 0):
            now = time.time()
            if now - last_no_hand_report > 2.0:
                print(f"No {args.hand} hand detected from {input_type} input yet.")
                last_no_hand_report = now
            return False
    ...
    def input_thread_fn():
        while not stop_event.is_set():
            try:
                ...
                processed = process_fingers_data(fingers_data)
            except KeyboardInterrupt:
                break
            except Exception:
                print("Input thread stopped after an exception:")
                traceback.print_exc()
                break
            if not processed:
                time.sleep(0.005)

    # OpenCV HighGUI preview is more reliable from the main thread.  In camera
    # mode we read and process frames inside the viewer loop instead of the
    # background input thread.
    synchronous_input = input_type == "camera"
    input_thread = (
        None
        if synchronous_input
        else threading.Thread(target=input_thread_fn, daemon=True)
    )
    ...
    try:
        ...
        while viewer.is_running():
            if synchronous_input:
                try:
                    process_fingers_data(input_device.get_fingers_data())
                except KeyboardInterrupt:
                    break
                except Exception:
                    print("Camera input stopped after an exception:")
                    traceback.print_exc()
                    break
