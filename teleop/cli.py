...
from .controller import enable_arm, input_check, preflight, run
...
def main() -> None:
    ...
    if args.command == "input-check":
        input_check(config)
        return
    control = "arm" if args.command == "arm-enable" else args.control
    ...
    action = "enable Nero" if args.command == "arm-enable" else "start command streaming"
